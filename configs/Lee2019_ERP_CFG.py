...
RUN_CFG = dict(
    ...
    steps=50001,  # number of training steps
    batch_size=128,  # batch size
    val_split=0.1,  # percentage of the dataset to use for validation
    device=th.device("cuda") if th.cuda.is_available() else th.device("cpu"),
    gradient_clip_val=1.0,
    checkpoint_freq=2000,  # checkpoint frequency in number of steps (based on train steps)
)
...
DATASET_CFG = dict(
    ...
    ),  # selected subjects，共54人，受试者 17 数据伪迹过多，故排除
    ch_names=[
        "Fp1",
        "Fp2",
        "F7",
